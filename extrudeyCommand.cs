...
{
    class extrudeyCommand : GenCommand
    {
        public override void ExecuteCmd(string szContent, ref GenContext Context, ref ArrayList Output, ref Object Tag)
        {
            ...
            if (!(Tag is SceneTree))
                throw new Exception("ExtrudeYCommand::ExecuteCmd:The last parameter must be a SceneTree instance");
            ...
            float y = (float)Convert.ToDouble(szPar2);
            if (Convert.ToInt32(szPar3) < 2)
                throw new Exception("ExtrudeYCommand:nLat must greater than 1 to create mayesurface");
            if (GenContext.GetNextParam(szContent, ref iPos, ',') != "")
                throw new Exception("ExtrudeYCommand:Invalid argument list.");

            int iCount = sct.m_Object3DList.Count + 1;
            if (szPar5 == "")
                szPar5 = "\"ExtrudeY" + iCount.ToString() + "\"";
            Object3D ob = sct.GetObject3D(Context.EvalText(szPar1));
            MayeArc arc = null;
            MayeSurface mayesurf = null;
            if (!(ob is MayeArc))
                throw new Exception("ExtrudeYCommand::ExecuteCmd:Only a MayeArc object can be extruded");
